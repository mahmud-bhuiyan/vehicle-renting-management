...
export class CustomerService {
  ...
  createCustomer(customer: RentCustomer): Observable<unknown> {
    return this.http
      .post(`${this.baseUrl}/CreateNewCustomer`, customer)
      .pipe(tap(() => this.afterCustomerMutation()));
  }
  ...
  clearBookingsCache(): void {
    ...
    this.bookingsLoading.set({});
    this.bookingsErrors.set({});
  }

  private afterCustomerMutation(): void {
    this.loadCustomers();
    this.dashboardService.loadDashboardData();
